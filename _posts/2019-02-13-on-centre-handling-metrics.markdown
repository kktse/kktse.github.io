...
computing handling metrics using our linear single-track vehicle model.
...
in our previous articles, but we did not explain the moitivation. Here is a bit
of an explaination:_

Ontario Time Attack is an IPAX equalized series. Modifications made to the
vehicle are equalized to offset the expected performance enhancement from the
modification. Therefore, the challenge of OTA is twofold: to maximize the vehicle
performance per unit performance index (PI), and to pilot the vehicle as
quickly as possible during the timed sessions.

In a competitive environment, simultaneously developing the car and driver is
extremely difficult. Our strategy is to use vehicle dynamic simulation in an
offline environment to develop the vehicle. This is advantageous because
vehicle development can continue without the need of track time.
...
Objective measurements are correlated to subjective evaluation.  Corolla et al.
show that frequency response metrics are highly correlated to subjective
...
# Experimental Setup
We will be evaluating the OTA #551 2012 Honda Civic Si for its on-centre
handling characteristics. All evaluation is performed in computer simulation
using the vehicle model developed in our previous articles.
...
deg SWA step. Four speeds are shown to demonstrate the effect of speed on the
response.
...
* **Settling Time** - time for signal to stay within a ±2% band of steady-state
* **Steady-State Gain** - the magnitude of the output over input when the signal is no longer changing

These metrics are visualized as following:
...
* The steady-state chassis slip angle gain crosses zero at around 50 km/h. This
  ...
  below this speed, partly due to the mathematical singularity at this point.
* The yaw rate and chassis slip angle rise and settle out at different times
  depending on speed. Below the tangent speed, the chassis slip angle is first
  to rise. However, above the tangent speed, the yaw rate is the first to rise.
...
As speed increases, we see an increasing resonant peak in the yaw rate
response. This is consistent with the step response metrics as the overshoot
and settling time also increased as speed increases. The chassis slip angle is
less resonant, however the steady-state response is out of phase above 70 km/h.
...
system dynamics as indicated by the same location of the system poles. The
variation in response is attributable to different placement of system zeros.
...
Lateral acceleration has two real zeros at low speeds. As the speed increases,
the zeros become complex conjugate pairs. These zeros become highly oscillatory
at high speeds.
...
open-loop response. Understanding the sensitivity to vehicle properties will
help us decide which modifications will have the biggest impact.

_A big thank you to OTA competitor Joseph Yang in the #551 2012 Honda Civic Si
for collaborating with us on this article. This would not be possible without
the support of Joseph Yang and his sponsors._
...
4. Gil Gómez, Gaspar. _Towards efficient vehicle dynamics evaluation using correlations of objective metrics and subjective assessments_. Diss. KTH Royal Institute of Technology, 2015.
