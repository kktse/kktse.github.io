...
properties can be measured with a set of floor scales. Consequently, we will
...
The diagonal terms represent the moments of inertia and the off-diagonal
terms represent the products of inertia. Because we assume the vehicle is
symmetric along the XZ-plane, we can set $$I_{xy} = 0$$ and $$I_{yz} = 0$$.
Based on these assumptions, this leaves us with four inertial parameters of
interest.
...
a variety of different passenger vehicles including different trims and
loading conditions. Measurements that include ballast in the vehicle are
excluded. If duplicate measurements were recorded for a given vehicle, only
the first measurement in the database is kept.
...
guess legend key along with a sample of vehicles of that type is shown below.
...
Given the data from the NHTSA Inertial Parameter Database, we can observe
trends with respect to vehicle mass. To begin, we will start by assessing
trends in the vehicle centre of gravity height.
...
From this graph, we can see that the CG height is likely to be higher when
the vehicle mass is greater. Notice the clustering of cars and trucks; trucks
and SUVs tend to have a higher CG height and tends to increase more with
vehicle mass. The table below summarizes the linear regression results.
...
This is because not all vehicles had their roll/yaw product measured.
...
The moments of inertia has good correlation with vehicle mass across all
vehicle types. The relationship scales relatively well with heavier vehicles
though there is some divergence particularly between minivans and SUVs. The
roll/yaw product will likely be positive except for pickup trucks. There is
some correlation for cars and SUVs but that relationship is weak. A summary
...
between SSF and vehicle mass. Upon further inspection, this appears to be
because of the clustering of cars and trucks. Trucks and SUVs tend to have a
lower SSF whereas cars which have higher SSF.
...
inertial properties of the vehicle. The yaw dynamic index has history
originating from the Olley "$$k^2$$" experiments conducted in the 1930s. The
yaw dynamic index defined as:
...
The application of such index is not prevalent for roll. From
experimentation, the following equation normalizes the roll inertial
measurements to a constant. Note that this equation is completely arbitrary
and is not based on any literature.
...
mass. These values are better described in terms of its average values since
they appear to centre around some constant value. These values are summarized
in the tables below.
...
vehicles tend to centre around a constant dynamic index and SSF. This means
that estimation of a vehicle's centre of gravity height and inertial
properties can be estimated using an average value. For cars, the following
...
The roll/yaw product can be estimated using the transfer function based on
the vehicle mass. Note that is subject to uncertainty because of the limited
data on this value and the weak correlation observed in the data.
...
of information required to describe a vehicle. With public databases, one can
identify trends in the industry to come up with a reasonable guess of any
given vehicle's centre of gravity position and inertial properties.

This analysis identified average values for the static stability factor and
the dynamic indices for cars and trucks. These values are non dimensionalized
to the vehicle geometry like track width and weight distribution. These
values are typically published by manufacturers and therefore makes it very
convenient computation. While there is a spread in these parameters, this
allows anyone to obtain a reasonable value without the needing access to
expensive vehicle measurement facilities.
